...
export const buildOptions = ({ cliOptions }: { cliOptions: CliOptions }) => {
  ...
  const apiDir = cliOptions.apiDir
    ? path.resolve(cliOptions.apiDir)
    : path.join(rootDir, "src", "api");
  const apiFile = cliOptions.apiFile
    ? path.resolve(cliOptions.apiFile)
    : path.join(rootDir, "src", "api", "index.ts");
